...
class MemoryController(
  ...
  for (c <- 0 until numChannels) {
    ...
    for (r <- 0 until numRequestors) {
      ...
      // increment age
      when(io.req.readReq(r).valid)  { readAge(r)  := readAge(r) + 1.U }
        .otherwise                   { readAge(r)  := 0.U }
      when(io.req.writeReq(r).valid) { writeAge(r) := writeAge(r) + 1.U }
        .otherwise                   { writeAge(r) := 0.U }
    }
